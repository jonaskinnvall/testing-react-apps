...
- Testing custom hooks and two methods of doing this
  - The custom hook component example were we create an example component that
    use the hook in a typical way so that we can test the component and thus
    test the hook indirectly
    - This method is useful in most cases e.g., when building an app
  - The fake component method were we create a fake component that doesn't
    return anything but uses the hook we want and we can use the result from our
    hook in our test. Allowing us to test the hook more directly
    - This method is useful if it is very hard to create an example component
      and is most useful for people developing libraries with hooks
- Creating a setup function to abstract away the common logic in our fake
  component so we can easily use it in multiple tests
- The renderHook from Testing Library that does what our setup function did and
  a little more
