...
- Using MSW to mock our requests to a backend server
  - Also that it can be used in the borwser for development and that was the
    initial idea for the library
- How we can set our msw server handlers up so that they can be used easily a
  cross our project
- How we can override these handlers in specific tests using the server.use
  function
  - In these cases it is **important** to resetHandlers() afterEach test
- Jest toMatchInlineSnapshot which makes it easier to maintain some assertions
  what might change in the code
  - If the text (or something else) changes in the code the test fails, but we
    can just click "u" in Jest watch mode and jest will update the snapshot and
    the test will pass
