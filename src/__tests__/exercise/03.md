...
- How to stay away from testing implementation details with React Testing
  Library
  - We can do this by using screen queries from testing-library which allows us
    to query for elements depending on e.g., their role, name or text instead of
    their order in the code etc.
- How userEvent can help us with that even further by triggering and thus
  "catching" a bunch of other events than what fireEvent does which makes our
  tests pass regardless of if a button is using onClick or onMouseDown e.g.
