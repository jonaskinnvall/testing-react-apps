...
- How to go about testing forms
  - How to test that the onSubmit function is called correctly in a
    implementation detail free way
- I learned about jest mock functions and how these can be utilized to make it
  easier for us to test e.g. that onSubmit function
  - With jest.fn() we don't care about the functions implementation but we can,
    by using some jest utilities, assert that the function was called with the
    correct arguments and that it was called the correct number of times
- Different ways of generating data for our tests
  - Good to do to indicate for others seeing the test that the data is not
    important, it can have the valeus of whatever (unless in some test it is
    important and then we can pass that value as an override)
  - Using Faker to generate data
  - using test-data-bot to generate data
