...
- Testing components that use context values
- The wrapper option from testing-library that let's us create a function that
  wrap the children with whatever wrapper we need (in this case a ThemeProvider)
- Creating our own render function that can use too clean up a lot of tests by
  just passing the UI we want to test to that function whcih wraps that UI with
  the Providers we want
  - This new render also takes differnet options we might test and adds them to
    the Provider or UI to render
- How Kent recommends setting our tests up using this type of render method that
  overrides the one from testing-library
- More specifically he recommends creating our own module (something like
  test-utils) which imports testing-library, creates our own render method and
  exports everything from testing-library and our own render method
  - So our tests should never import testing-library, it should import our own
    utility module which forwards everything from testing-library
