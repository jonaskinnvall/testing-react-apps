...
- Mocking browser APIs
  ...
  - Sometimes we might be using a browser API that is not supported in jsdom
    (which is used to run our tests in node and simulate a browser) and in those
    cases we might have to mock that function which is used
  - We make a mock function using jest.fn for the function we want to mock and
    then use mockImplementation to make it do what we want in our test

- Mocking modules
  - This is another alternative to mocking the browser API
  - To do this we use the jest.mock API which mocks the functions in the
    specified module so we can use mockImplementation to make it do what we want
    for a ncie test experience
  - Sometimes the browser API might be too hard to mock, or we just choose to
    rely on the tests set up for that API (or which ever third party module)
  - This can also bu used if we just don't want our (or third party) module to
    run in our tests
  -
